...
def check(json,status,mode):
    ...
    for i in range(len(list_2)):
        ...
        if mode == "direction":
            a = 0
            for j in list_2[i][0]:
                if re.findall(r"^ *eastern *$|^ *western *$|^ *southern *$|^ *northern *$|^ *central *$|^ *northeastern *$|^ *northwestern *$|^ *southeastern *$|^ *southwestern *$|^ *insular *$|^ *canadian *$|^ *Costeño *$|^ *Llanero *$|^ *Mainland *$"                              ,j,re.I|re.M)!=[]:
                    a += 1
                if a > 0:
                    if dictionary[list_2[i][1]] not in return_list:
                        return_list.append(dictionary[list_2[i][1]])
...
def formTree(json,status):
    ...
    return tree      
tree = formTree("Indo-European language data.json",status="classification")
...
def show_simple_tree(json,status,start=None,branch=None):
    ...
    final_unadded =     ", ".join(list(set(simplelangTree.langList.keys())-set([tree.nodes[k].tag for k in tree.nodes])))
...
def show_complex_tree(json,status,start=None,branch=None):
    ...
    while (len(simplelangTree.langList)-len(added_list)) > 0: 
        ...
        unadded_num_list.append(len(simplelangTree.langList)-len(added_list))
        print(str(len(simplelangTree.langList)-len(added_list))+" languages haven't been added.")
    final_unadded =     ", ".join(list(set(simplelangTree.langList.keys())-set(list(G.nodes))))
    print("These languages are not added: "+final_unadded+"\n\n")
    depth = 0
...
def lang_graph(lang_dist_json,lang_list_json,threshold=1):
    ...
    nx.draw(G, alpha=1, node_size=10, with_labels=True)  
...
def lang_dendrogram(lang_dist_json,lang_list_json,json=None,start=None):
    import numpy as np
